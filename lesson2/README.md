...
| 3 |  [example.sql](https://github.com/HENRYHKll/gb_basics_relational_db_MySQL/blob/main/lesson2/example.sql) |
| 4 |  [100line.sql](https://github.com/HENRYHKll/gb_basics_relational_db_MySQL/blob/main/lesson2/100line.sql) |
...
+-----------+------------------+
5 rows in set (0.00 sec)
mysql> ALTER USER 'root'@'%' IDENTIFIED WITH caching_sha2_password BY '11'; ### не понял почему, но пароль при установки не работал пришлось обновить
...
```
Вроде ничего не пропустил все работает
...
mysql> USE example
mysql> CREATE TABLE users (id INT UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT, name  VARCHAR (255));
...
mysql> INSERT INTO users (name) VALUES ('Issac');
mysql> INSERT INTO users VALUES (0, 'Анна');
mysql> INSERT INTO users VALUES (NULL, 'Иван');
...
-- Dump completed on 2021-03-30 13:02:57
--
...
INSERT INTO `users` VALUES (1,'Issac'),(2,'Анна'),(3,'Иван');
...
-- Dump completed on 2021-03-30 12:57:41
```


## 4.(по желанию) Ознакомьтесь более подробно с документацией утилиты mysqldump. Создайте дамп единственной таблицы help_keyword базы данных mysql. Причем добейтесь того, чтобы дамп содержал только первые 100 строк таблицы.e
...
-- Dump completed on 2021-03-30 13:13:12
```
